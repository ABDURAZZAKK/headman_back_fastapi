...
async def get_hws_by_category_id(
    category_id: int, hwRepo: HomeworkRepository = Depends(get_homework_repository)
):
    return await hwRepo.get_by_category_id(category_id)


@router.get("/attached_files/{hw_id}")
async def get_attached_file_names(
    hw_id: int, hwRepo: HomeworkRepository = Depends(get_homework_repository)
):
...
async def get_file(dir_path: int, file_name: str):
    ...
    return FileResponse(
        f"{HW_ATTACHED_PATH}\\{dir_path}\\{file_name}",
        filename=file_name,
...
async def attach_hw_file(
    ...
    if current_user and hw:
        if hw.creater == current_user.id:
            if hw.path_to_files is None:
                await hwRepo.update(
                    hw.id, d={"path_to_files": file_service.create_timestamp_dir()}
                )
    ...
    raise HTTPException(
        status_code=status.HTTP_404_NOT_FOUND, detail="Not found homework"
    )
